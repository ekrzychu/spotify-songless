      ...
      {Array.from({ length: MAX_ATTEMPTS }, (_, index) => {
        ...
        return (
          <li className={`attempt attempt--${state}`} key={index} aria-current={state === "current" ? "step" : undefined}>
            <span className="attempt-mark" aria-hidden="true">{state === "correct" ? "✓" : state === "incorrect" ? "×" : state === "skipped" ? "—" : index + 1}</span>
            <span>{attempt?.label ?? (state === "current" ? `Attempt ${index + 1}` : "")}</span>
          </li>
        );
      })}
    </ol>
